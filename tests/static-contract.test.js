...
const html=read('index.html'); const baseApp=read('app.js'); const validationApp=read('m01-validation-app.js'); const validationStyles=read('m01-validation.css'); const labData=read('m01-learning-lab-data.js'); const artDirection=read('art-direction.css');
...
test('index loads validation data and canonical domains before base app then validation extension',()=>{
  ...
  const scripts=['course-data.js','m01-validation-data.js','m01-learning-lab-data.js','course-learning-labs-v7.js','module-practice-domain-v7.js','mastery-domain-v7.js','app.js','m01-validation-app.js'];
...
test('base router reserves M01 validation route for the extension',()=>{
  assert.match(baseApp,/route:\s*['"]validation-m01['"]/); assert.match(baseApp,/if \(route === ['"]validation-m01['"]\) return/);
});
...
test('M01 lab data loads before canonical normalizer/app and keeps stable cold IDs',()=>{
  assert.ok(html.indexOf('m01-learning-lab-data.js')>html.indexOf('m01-validation-data.js')); assert.ok(html.indexOf('m01-learning-lab-data.js')<html.indexOf('course-learning-labs-v7.js')); assert.ok(html.indexOf('course-learning-labs-v7.js')<html.indexOf('app.js'));
  for(const token of ['project-system','system-diagnostic','m01-drill-system','m01-drill-diagnostic','workbookFields','transferPrompt'])assert.ok(labData.includes(token));
});
